...
import { MINIO_CONFIG, generateFileName } from "../../lib/minio";
...
interface ChatDetailProps {
  projectId: string;
  onProjectInfoLoaded?: (info: ProjectInfo) => void;
}

export default function ChatDetail({ projectId, onProjectInfoLoaded }: ChatDetailProps) {
  ...
  useFocusEffect(
    useCallback(() => {
      if (projectId && profile?.id && messages.length > 0) {
        const lastMessage = messages[messages.length - 1];
        markAsRead(lastMessage.id);
        fetchOtherUsersReadStatus();
      }
    }, [projectId, profile?.id, messages, markAsRead, fetchOtherUsersReadStatus])
  );
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: `minor_project_id=eq.${projectId}`,
        },
        async (payload) => {
  ...
  }, [projectId, markAsRead, fetchOtherUsersReadStatus]);
  ...
  const uploadImageToMinio = async (image: {
    ...
    try {
      const path = `chat/${projectId}/${image.name}`;
  ...
  const handleSend = async () => {
    if ((!newMessage.trim() && !selectedImage) || !projectId || !profile?.id) return;
